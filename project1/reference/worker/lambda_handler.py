...
Performance notes:
    - Results are posted to SQS before writing to DynamoDB. The SQS post
      is on the measured latency path (processed_at timestamp).
    - DynamoDB writes are fired in a background daemon thread immediately
      after the SQS result post, so they do NOT add to measured latency.
      Lambda freezes the container on return, but AWS typically lets the
      in-flight write complete before the freeze takes full effect.
...
def process_opportunity(opportunity):
    ...
    # Step 2: Persist to DynamoDB off the hot path.
    # We fire the write in a daemon background thread so it does not block
    # the return of process_opportunity() or add to measured latency.
    # Lambda freezes the container after the handler returns, but the
    # in-flight write typically completes before the freeze settles.
    def _write_to_dynamo(record):
        try:
            table = dynamodb.Table(DYNAMO_TABLE_NAME)
            table.put_item(
                Item={
                    "opportunity_id": record["opportunity_id"],
                    "winning_advertiser_id": record["winning_advertiser_id"],
                    "winning_bid_amount": Decimal(str(record["winning_bid_amount"])),
                    "winning_score": Decimal(str(record["winning_score"])),
                    "score_margin": Decimal(str(record["score_margin"])),
                    "processed_at": record["processed_at"],
                }
            )
        except Exception as exc:  # noqa: BLE001
            logger.error("DynamoDB write failed for %s: %s",
                         record["opportunity_id"], exc)

    threading.Thread(
        target=_write_to_dynamo,
        args=(result_record,),
        daemon=True,
    ).start()
